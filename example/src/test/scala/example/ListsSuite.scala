...
class ListsSuite extends FunSuite {
  ...
  test("sum of a few negative numbers") {
    assert(sum(List(-2, -2, -5, -4)) === -13)
  }
  
  test("sum of a few positive and  negative numbers") {
    assert(sum(List(-2, 6, -5, -4, 7)) === 2)
  }
  ...
  test("max of a few negative numbers") {
    assert(max(List(-5,-1,-3)) === -1)
  }
  
  test("max of a few positive and negative numbers") {
    assert(max(List(-5,4,7)) === 7)
  }
}
